...
def _print_verdict(result) -> None:
    ...
    if not result.meaningful:
        ...
        print(f"    Correct mock: {result.passed_correct}  |  Broken impl: {result.failed_broken}")
...
def main() -> int:
    ...
    with (
        ...
        fixed_verdicts = gate2.filter_meaningful(fixed_candidates, correct_url, broken_url)
  ...
  └────────────────────────────────────────────────────────────┘
        ...
        if len(caught) == 0:
            print("ERROR: demo did not catch any cheats — check fixture setup", file=sys.stderr)
            return 1
        return 0
