...
def test_unknown_message_type():
    ...
    bad_json = json.dumps(
        {"type": "unknown_message", "protocol_version": "1.0.0", "payload": {}}
    )
...
def test_missing_protocol_version():
    ...
    bad_json = json.dumps({"type": "truth_fragment", "payload": {}})
...
def test_major_version_mismatch():
    ...
    bad_json = json.dumps(
        {"type": "truth_fragment", "protocol_version": "2.0.0", "payload": {}}
    )
...
def test_ack_message():
    ...
    msg = ProtocolMessage(
        type=ProtocolMessageType.ACK, payload={"status": "received", "trace_id": "t-1"}
    )
    serialized = dumps(msg)
    restored = loads(serialized)
