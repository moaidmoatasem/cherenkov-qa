...
import importlib, os, sys, json, tempfile, subprocess
...
def test_gp2_ingest_stage_accepts_spec():
    ...
    with tempfile.NamedTemporaryFile(suffix=".json", mode="w", delete=False) as f:
        json.dump(spec_data, f); spec_path = f.name
...
def test_gp2_contracts_are_versioned():
    ...
    from cherenkov.core.contracts import IngestOutput, PlanOutput, GenerateOutput, ReviewOutput
...
def test_gp3_generate_output_has_test_code_field():
    ...
    fields = getattr(GenerateOutput, "model_fields", None) or getattr(GenerateOutput, "__fields__", {})
...
def test_gp4_ejected_tests_have_no_cherenkov_imports():
    ...
    for ts_file in fixtures_dir.glob("*.spec.ts"):
        ...
        assert "cherenkov" not in content.lower(), (
...
def test_gp5_oracle_fail_on_wrong_status():
    ...
    with patch.object(prism, "evaluate", return_value={"is_correct": False, "confidence": 0.0, "reason": "status mismatch"}):
...
def test_gp5_oracle_no_fail_open():
    ...
    for i, line in enumerate(lines):
        if "except" in line:
            in_except = True
        if in_except and "is_correct" in line and "True" in line and "False" not in line:
...
def test_gp6_chat_agent_no_fake_verdict():
    ...
    try:
        ...
        assert '"verdict"' not in source or "error" in source.lower(), (
...
def test_gp7_metrics_endpoints_registered():
    ...
    assert "/api/v1/metrics/pipeline" in routes, f"Missing /api/v1/metrics/pipeline. Routes: {routes}"
    assert "/api/v1/metrics/prometheus" in routes, f"Missing /api/v1/metrics/prometheus. Routes: {routes}"
...
def test_gp10_k8s_crd_has_device_targets():
    ...
    crd_path = REPO_ROOT / "operator" / "config" / "crd" / "bases" / "validation.cherenkov.io_conformancechecks.yaml"
