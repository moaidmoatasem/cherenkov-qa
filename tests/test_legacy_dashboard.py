...
def main():
    ...
    if not healthy:
        ...
        if os.path.exists("dashboard_startup.log"):
            with open("dashboard_startup.log") as f:
                print(f.read())
        if os.path.exists("dashboard_startup.err"):
            with open("dashboard_startup.err") as f:
                print(f.read())
    ...
    try:
        ...
        r = requests.post(
            ...
            json={"scenario_id": "happy_path", "reason": "ok"},
            timeout=15,
        )
        check(
            "POST /review/approve (200)",
        ...
        r = requests.post(
            ...
            json={"scenario_id": "dummy", "reason": "test"},
            timeout=15,
        )
        check(
            "POST /review/reject (200)",
            r.status_code == 200 and r.json().get("status") == "rejected",
        )

        r = requests.post(
            f"{base_url}/api/v1/review/edit", json={"scenario_id": "dummy"}, timeout=15
        )
        check("POST /review/edit (400 no code)", r.status_code == 400)

        r = requests.post(
            f"{base_url}/api/v1/review/edit",
            json={"scenario_id": "edit_test", "test_code": "test('x', () => {});"},
            timeout=15,
        )
        check(
            "POST /review/edit (200 saved)",
            r.status_code == 200 and r.json().get("status") == "saved",
        )
        ...
        r = requests.post(
            f"{base_url}/api/v1/eject",
            json={"output_path": os.path.abspath("temp_eject_out")},
            timeout=15,
        )
        check(
            "POST /eject (200)",
            r.status_code == 200 and r.json().get("status") == "ejected",
        )
        ...
        divs = r.json()
        check(
            "GET /divergences (200 list)",
            r.status_code == 200 and isinstance(divs, list) and len(divs) > 0,
        )

        r = requests.post(
            f"{base_url}/api/v1/divergences/act",
            json={"divergence_id": divs[0]["id"], "action": "mark_intended"},
            timeout=15,
        )
        check(
            "POST /divergences/act (200 rejected)",
            r.status_code == 200 and r.json().get("new_status") == "rejected",
        )

        r = requests.post(
            f"{base_url}/api/v1/divergences/act",
            json={"divergence_id": "D-99", "action": "reject"},
            timeout=15,
        )
