...
def main():
    ...
    try:
        ...
        report = scanner.run_compliance_audit(
            target_url="http://127.0.0.1:8000", spec_path="stub/target_spec.json"
        )

        # 4. Verify outputs
        assert os.path.exists(
            report_file
        ), "Compliance audit report JSON was not written to disk."
        ...
        assert (
            "SAMA_CCSF" in mappings
        ), "Report is missing SAMA CCSF framework mapping details."
        assert (
            "EGYPT_FinCSF" in mappings
        ), "Report is missing CBE FinCSF framework mapping details."
