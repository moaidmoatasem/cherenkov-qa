...
BASELINE_DIRS = [
    ...
    os.path.join(REPO, 'stub', 'generated_tests', 'visual_cli_default.spec.ts-snapshots'),
]
SCRATCHPAD_SPEC = os.path.join(REPO, 'stub', 'generated_tests', 'visual_cli_default.spec.ts')
...
def main():
    ...
    try:
        ...
        assert r1.returncode == 0, f'Pass 1 (baseline init) must exit 0 — got {r1.returncode}'
        ...
        assert r2.returncode == 0, f'Pass 2 (compare) must exit 0 against own baseline — got {r2.returncode}'
        _assert_no_user_tests_modified(before, _snapshot_user_tests())
