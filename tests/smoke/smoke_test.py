...
def run_failure_path():
    ...
    assert (
        not success
    ), "Pipeline did not abort as expected under tripped circuit breaker."
    assert (
        engine.breaker.tripped
    ), "Circuit breaker failed to trip after failure limit was reached."
    assert (
        engine.breaker.error_count == 1
    ), f"Expected 1 failure on circuit breaker, got {engine.breaker.error_count}"
