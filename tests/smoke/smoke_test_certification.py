...
check(len(_DEFAULT_GOLD_SET_ITEMS) >= 3, f"at least 3 items (got {len(_DEFAULT_GOLD_SET_ITEMS)})")

for item in _DEFAULT_GOLD_SET_ITEMS:
    check("prompt" in item, f"item has prompt: {item['prompt'][:40]}")
    check("expected_contains" in item, f"item has expected_contains")
...
check(0.0 <= scores["context_relevance"] <= 1.0, f"context_relevance in range: {scores['context_relevance']}")
check(0.0 <= scores["answer_faithfulness"] <= 1.0, f"answer_faithfulness in range: {scores['answer_faithfulness']}")
check(0.0 <= scores["answer_relevance"] <= 1.0, f"answer_relevance in range: {scores['answer_relevance']}")
