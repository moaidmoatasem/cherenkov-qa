...
def test_auth_expiry_detection():
    ...
    diagnoser.record_passing_snapshot(
        scenario_id, status=201, body={"id": 42, "email": "test@example.com"}
    )
    ...
    assert (
        diag.failure_class == FailureClass.AUTH_EXPIRY
    ), f"Expected AUTH_EXPIRY, got {diag.failure_class}"
    ...
    assert (
        "BEARER_TOKEN" in suggestion["suggestion"]
    ), "Auth expiry suggestion missing key tokens"
...
def test_contract_drift_detection():
    ...
    assert (
        diag.failure_class == FailureClass.CONTRACT_DRIFT
    ), f"Expected CONTRACT_DRIFT, got {diag.failure_class}"
    assert "email" in diag.missing_fields, "Failed to identify missing field 'email'"
    print(
        "[PASS] Diagnoser correctly classified FailureClass.CONTRACT_DRIFT and isolated missing field"
    )
    ...
    assert (
        "[RED REGRESSION]" in suggestion["suggestion"]
    ), "Contract drift suggestion failed to flag RED regression"
    assert (
        "expect(data).toHaveProperty" in suggestion["suggestion"]
    ), "Contract drift suggestion missing suggested assertions"
...
def verify_suggest_only_trust_rule(initial_modified):
    ...
    assert (
        len(new_modified) == 0
    ), f"Violation: Test files were auto-modified during run: {new_modified}"
    print(
        "[PASS] Git status is 100% clean relative to run start -- zero files were auto-modified by healing! Suggest-only trust rule honored.\n"
    )
