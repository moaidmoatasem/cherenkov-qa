...
def main() -> int:
    ...
    checks["list: schema_version=hitl/v1"] = (
    ...
    checks["show: item.endpoint correct"] = (
        data["payload"]["item"]["endpoint"] == "/users/{id}"
    )
    ...
    checks["show not_found: error.code=not_found"] = (
        data_nf.get("error", {}).get("code") == "not_found"
    )

    # ── approve alice (JSON) ───────────────────────────────────────────────
    rc, data = capture_json(
        run_approve, "ck_smoke_1", actor="@alice", json_out=True, db_path=db
    )
    ...
    checks["approve: schema_version correct"] = (
        data.get("schema_version") == SCHEMA_VERSION
    )
    ...
    with contextlib.redirect_stderr(stderr_buf2):
        rc_c, data_c = capture_json(
            run_approve, "ck_smoke_1", actor="@bob", json_out=True, db_path=db
        )
    checks["conflict: rc=1"] = rc_c == 1
    checks["conflict: ok=False"] = data_c.get("ok") is False
    checks["conflict: error.code=conflict"] = (
        data_c.get("error", {}).get("code") == "conflict"
    )
    ...
    checks["post-resolve list-all: 2 total"] = (
        rc_all == 0 and data_all["payload"]["count"] == 2
    )
    ...
    with mp.dict(os.environ, {"USER": "env_tester"}):
        rc_env, data_env = capture_json(
            run_approve, "ck_env_actor", actor=None, json_out=True, db_path=db
        )
