...
# Test dry-run mode
def test_maestro_dry_run_is_skipped_not_passed():
    """A dry run touches no device, so it must not report a passing verdict."""
...
def test_appium_dry_run_is_skipped_not_passed():
    """A dry run touches no device, so it must not report a passing verdict."""
    runner = AppiumRunner()
...
def test_maestro_run_directory_dry_run():
    """Directory dry runs are skipped for the same reason single flows are."""
    runner = MaestroRunner()
