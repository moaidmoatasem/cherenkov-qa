...
class TestSQLiteKnowledgeRepository(unittest.TestCase):
    ...
    def test_store_and_query(self):
        item = KnowledgeItem(item_id="t1", source="verdicts", data={"endpoint": "/users"})
    ...
    def test_search_finds_pattern(self):
        self.repo.store(KnowledgeItem(item_id="1", source="verdicts",
                                       data={"endpoint": "/users", "status": "auth timeout"}))
        self.repo.store(KnowledgeItem(item_id="2", source="verdicts",
    ...
    def test_search_returns_empty_for_nonexistent(self):
        self.repo.store(KnowledgeItem(item_id="1", source="verdicts", data={"x": "hello"}))
        results = self.repo.search("nonexistent")
        self.assertEqual(len(results), 0)

    def test_get_by_id_found(self):
        self.repo.store(KnowledgeItem(item_id="t1", source="verdicts", data={"endpoint": "/api"}))
    ...
    def test_query_limit(self):
        for i in range(5):
            self.repo.store(KnowledgeItem(item_id=f"t{i}", source="verdicts", data={"i": i}))
    ...
    def test_search_with_limit(self):
        for i in range(5):
            self.repo.store(KnowledgeItem(item_id=f"t{i}", source="test", data={"text": f"hello {i}"}))
    ...
    def test_store_override(self):
        self.repo.store(KnowledgeItem(item_id="t1", source="verdicts", data={"version": 1}))
        self.repo.store(KnowledgeItem(item_id="t1", source="verdicts", data={"version": 2}))
        result = self.repo.get_by_id("t1")
    ...
    def test_search_pattern_in_json_value(self):
        self.repo.store(KnowledgeItem(item_id="1", source="test",
    ...
    def test_multiple_sources(self):
        for i in range(3):
            self.repo.store(KnowledgeItem(item_id=f"v{i}", source="verdicts", data={"i": i}))
        for i in range(3):
            self.repo.store(KnowledgeItem(item_id=f"f{i}", source="feedback", data={"i": i}))
...
class TestGraphRAG(unittest.TestCase):
    ...
    def test_query_with_data(self):
        self.repo.store(KnowledgeItem(item_id="v1", source="verdicts",
    ...
    def test_query_respects_limit(self):
        for i in range(5):
            self.repo.store(KnowledgeItem(item_id=f"v{i}", source="verdicts",
                                           data={"i": i}))
        for i in range(5):
            self.repo.store(KnowledgeItem(item_id=f"f{i}", source="feedback",
                                           data={"i": i}))
    ...
    def test_explain_divergence(self):
        ...
        self.repo.store(KnowledgeItem(item_id="i1", source="idioms",
                                       data={"endpoint": "/users", "method": "GET"}))
