...
def test_openai_vlm_provider():
    with patch.object(get_settings(), "TIER_VISION_PROVIDER", "openai"), patch("cherenkov.substrate.provider.OpenAIInferenceClient") as mock:
...
def test_localai_vlm_provider():
    with patch.object(get_settings(), "TIER_VISION_PROVIDER", "localai"), patch("cherenkov.substrate.providers.localai.LocalAIVLMProvider") as mock:
        mock.return_value = MagicMock()
        p = get_vlm_provider()
        assert p is not None
...
def test_unknown_provider_raises():
    with patch.object(get_settings(), "TIER_VISION_PROVIDER", "unknown"), pytest.raises(ValueError):
        get_vlm_provider()
...
def test_resolve_auto_local_docker():
    with patch.object(get_settings(), "TIER_VISION_PROVIDER", "auto"), patch("cherenkov.core.devices.DeviceInfo") as mock_info:
        info = MagicMock()
        info.vlm_tier = "local"
