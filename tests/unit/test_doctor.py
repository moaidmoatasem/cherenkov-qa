...
def test_doctor_all_flags():
    """Verifies that --device, --vlm, and --localai flags route to the correct detectors."""
    runner = CliRunner()
    _dev = {"device_class": "desktop", "vlm_tier": "local", "has_gpu": False,
            "has_docker": True, "os_name": "Windows", "cpu_count": 8, "memory_gb": 16.0}
    _vlm = {"available": False, "model": "", "error": "no ollama"}
    _lai = {"available": False, "model": "", "error": "no localai"}
    with patch("cherenkov.substrate.doctor._detect_device", return_value=_dev), \
         patch("cherenkov.substrate.doctor._detect_ollama_vlm", return_value=_vlm), \
         patch("cherenkov.substrate.doctor._detect_localai_vlm", return_value=_lai):
