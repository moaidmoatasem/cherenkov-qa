...
def test_qwen_code_agent_success():
    with patch("shutil.which", return_value="/usr/bin/qwen"), \
        ...
        args = RunQwenCodeAgentArgs(prompt="test prompt", context="test context", files=["a.py"])
...
def test_qwen_code_agent_timeout():
    with patch("shutil.which", return_value="/usr/bin/qwen"), \
         patch("subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="qwen", timeout=120)):
