...
def run_advanced_healing_smoke_tests():
    ...
    assert result_seq.failure_class == FailureClass.STATE_SEQUENCE, "Failed to classify state sequence issue."
    print("✓ Stateful sequence classification verified (404 mapped to STATE_SEQUENCE).\n")
    ...
    assert result_flake == FailureClass.FLAKY_SUCCESS, "Failed to diagnose flaky test pass."
    ...
    assert result_fail == FailureClass.DETERMINISTIC_FAILURE, "Failed to diagnose deterministic failure."
    assert fail_count == 2, f"Expected 2 retry test executions, got {fail_count}"
    print("✓ Deterministic failure retry classification verified (consistently failed).\n")
