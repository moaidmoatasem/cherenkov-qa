...
def resolve_refs_depth(
    ...
    """Collect the schemas a node references, walking ``$ref`` up to a depth.

    Args:
        node: Any part of a parsed spec — dict, list or scalar.
        schemas (dict): The spec's ``components.schemas`` table.
        resolved (dict): Accumulator, mutated in place with the schemas found.
        depth (int): Current recursion depth.
        max_depth (int): Depth ceiling, which also bounds ``$ref`` cycles.
    """
    if depth > max_depth:
        return
