    ...
    for _ in range(timeout):
        try:
            with urllib.request.urlopen(url, timeout=1) as response:
                if response.status == 200 or response.status == 404 or response.status == 500:
...
def main():
    ...
    subprocess.run(["npx", "playwright", "install", "chromium"], cwd=dashboard_dir, check=True)
    ...
    backend_log = open(os.path.join(project_root, "backend_test.log"), "w", encoding="utf-8")
    backend_err = open(os.path.join(project_root, "backend_test.err"), "w", encoding="utf-8")
    ...
    frontend_log = open(os.path.join(project_root, "frontend_test.log"), "w", encoding="utf-8")
    frontend_err = open(os.path.join(project_root, "frontend_test.err"), "w", encoding="utf-8")
