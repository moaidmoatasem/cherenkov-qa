...
import { parseSarif } from '../../backend/sarif';
...
suite('parseSarif', () => {
    test('returns empty array for empty input', () => {
        assert.deepStrictEqual(parseSarif(''), []);
    });

    test('returns empty array for non-SARIF text', () => {
        assert.deepStrictEqual(parseSarif('hello world\nsome output\n'), []);
    });

    test('returns empty array for SARIF with no results', () => {
        const result = parseSarif(VALID_SARIF_NO_RESULTS);
        assert.strictEqual(result.length, 0);
    });

    test('parses violations from valid SARIF JSON', () => {
        const violations = parseSarif(VALID_SARIF_VIOLATIONS);
    ...
    test('parses SARIF embedded in surrounding CLI output text', () => {
        ...
        const violations = parseSarif(surrounding);
        assert.strictEqual(violations.length, 2);
    });
});
