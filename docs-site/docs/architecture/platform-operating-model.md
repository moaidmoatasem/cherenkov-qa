...
description: The product and architecture contract behind CHERENKOV — one independent quality verdict across many evidence sources, runners, and models, with QA retaining policy and release authority.
---

# Platform Operating Model

!!! abstract "Status"
    **Directional architecture and product contract — not a delivery roadmap.**
    See [System Design](system-design.md) for what ships today and [User Journeys](user-journeys.md) for how it feels in practice.
...
!!! info "What ships today"
    API conformance is the flagship, **shipped** `EvidenceSource` + `EvidenceExecutor` + `Oracle` — `validate`, `verify`, generation, the [check-suite integrity audit](../guides/check-suite.md), and [signed certificates](../guides/certificates.md). Eight-plus `ModelProvider`s (Ollama, LocalAI, OpenAI, and more) and the MCP `Connector` are live. The other contracts above are the platform's direction, not a claim of current coverage.
...
Agents may propose a memory promotion. They may not silently turn an unreviewed observation into team truth. A later verdict can supersede a previous memory record; history must remain auditable. This is the governance layer over the [Second Brain knowledge mesh](second-brain.md).
