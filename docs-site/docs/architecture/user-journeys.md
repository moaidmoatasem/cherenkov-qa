---
title: User Journeys
description: Five end-to-end journeys that make CHERENKOV legible from a user's point of view — QA onboarding, agent verification, cross-surface release risk, incident learning, and governance.
---

# User Journeys

!!! abstract "Status"
    **Product journeys and acceptance intent — not a claim that every step is shipped today.**
    Read the [Platform Operating Model](platform-operating-model.md) for the architecture behind these flows.
