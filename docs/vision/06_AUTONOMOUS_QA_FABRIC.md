...
| 4 | **Reward-hacking built anyway.** Self-play + committee looked rigorous in docs but agents still produced plausible-passing junk at scale. | Medium | rising false-positive KPI; humans stop trusting findings | E12 governance KPIs (false-positive rate, defect-escape) tracked from E7, not added last; no-shared-context verifier is non-optional. |
| 5 | **Manual-QA pillar stayed a slogan.** We built agents but never the concrete authoring/triage UX, so the "empower manual QAs" promise never materialized. | Medium | no non-coder has authored a passing test by E10 | E10 exit *requires* a non-coder round-trip; design the UX before the agent. |
