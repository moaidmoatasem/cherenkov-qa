...
> **The Validation Gate eats first.** Track A (OpenAPI → Playwright conformance) must pass a real **5-QA validation gate** before E9–E13 frontier work or any external voice layer (OpenClaw, dashboard authoring) is allowed to consume it. Surfaces past the gate are labelled `do-not-extend-until-gate` and `blocked:validation-gate`. See [`HANDOVER.md`](https://github.com/moaidmoatasem/cherenkov-qa/blob/main/HANDOVER.md) and [`AGENTS.md`](https://github.com/moaidmoatasem/cherenkov-qa/blob/main/AGENTS.md).
