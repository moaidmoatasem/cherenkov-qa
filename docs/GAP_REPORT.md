...
* **Raw Evidence & Code References:**
  - **Works:** The tuned generator prompt is committed to `prompts/generator_system.txt` and loaded once at import in `cherenkov/stages/generate.py` into the `SYSTEM_PROMPT` static constant (preserving the prefix-cache optimization on Ollama, Delta D10 / V1). An optional `CHERENKOV_GENERATOR_PROMPT` env var overrides the path.

### Phase 2 — Weeks 2-3 — Core Infrastructure
* **Status:** `PARTIAL`
* **Raw Evidence & Code References:**
  - **Works:** Type-safe, versioned Pydantic contracts exist for every boundary (`IngestOutput`, `PlanOutput`, `GenerateOutput`, `ReviewOutput`) in `cherenkov/core/contracts.py`. Structured logger emitting JSONL to stderr exists in `cherenkov/core/errors.py`. Ollama client with format constraint is implemented in `cherenkov/ai/ollama_client.py`.
...
* **Raw Evidence & Code References:**
  ...
  - **Structural Difference:** There is no separate `cli/progress.py` file. CLI progress spinners and dynamic carriage returns are directly printed inside `cherenkov/core/orchestrator.py` using terminal escape formatting.

### Phase 4 — Weeks 6-7 — INGEST (real)
* **Status:** `PARTIAL`
* **Raw Evidence & Code References:**
  - **Works:** `cherenkov/stages/ingest.py` parses specs, resolves `$ref` with a depth constraint (`SCHEMA_DEPTH`), calculates richness scores, and builds a deterministic mutation menu (missing fields, extreme strings, numeric boundaries).
...
* **Raw Evidence & Code References:**
  ...
  - **Incomplete/Missing:** The plan states: *"`stages/plan.py` (deepseek-r1:8b): emit scenarios... SELECT from the menu"*. The actual `cherenkov/stages/plan.py` is simple, deterministic Python code that maps the Ingest mutation menu straight into `Scenario` contracts. No LLM connection or planning inference is executed in the PLAN stage.
...
* **Status:** `PARTIAL`
* **Raw Evidence & Code References:**
  - **Works:** `cherenkov/stages/review.py` executes cheap-first static gates: Syntax (markdown strip), Structure (imports verification), AST-validate (zero axios/fetch bleed), TSC compile (`tsc --noEmit`), and Prism dynamic mock server dry-run. Threshold verdicts (Auto-Approve / HITL / Regenerate) are strictly enforced.
...
* **Status:** `WORKS` (gap closed — #100)
* **Raw Evidence & Code References:**
  - **Works:** `cherenkov/healing/diagnose.py` reads/writes passing snapshots under `.cherenkov/snapshots/`, classifies failure classes (`AUTH_EXPIRY`, `CONTRACT_DRIFT`), and prints suggestions without auto-committing code alterations. Isolated sandbox healer is supported.
...
* **Raw Evidence & Code References:**
  ...
  - **Structural Difference:** Consolidated directly inside `cherenkov/execution/validate.py` instead of separating into `validate/real_server.py` and `validate/value_tightening.py`.
...
* **Raw Evidence & Code References:**
  ...
  - **Structural Difference:** Implemented in `cherenkov/execution/eject.py` instead of `eject/exporter.py` to keep internal module boundaries cohesive.

### Phase 11 — Weeks 18-19 — Dashboard (defer-first)
* **Status:** `WORKS / DEFERRED`
* **Raw Evidence & Code References:**
  - **Works:** React Dashboard E2E components (review screen, healing screen, WS integration) exist inside `track-b-c-deferred/dashboard` and `track-b-c-deferred/cherenkov/api/main.py`. They are cleanly quarantined off the active Track A runtime surface.
