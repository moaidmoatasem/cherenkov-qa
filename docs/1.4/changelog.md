...
### Added
- **Coverage Map API** (Phase 14): `build_coverage_map()` in `cherenkov/web/coverage_map.py` computes per-endpoint conformance coverage (`GET /api/v1/coverage/map`).
- **Continuous Conformance Trend** (Phase 14): `conformance_trend()` and `conformance_summary()` stream verdict/divergence time-series (`GET /api/v1/coverage/conformance-trend`, `GET /api/v1/coverage/conformance-summary`).
- **Regression Detection** (Phase 14): `detect_regressions()` flags verdict downgrades, coverage regressions, and divergence spikes (`GET /api/v1/coverage/regressions`).
- **GitHub PR Coverage Diff Bot** (Phase 14): `format_coverage_comment()` posts automated coverage diff analysis on pull requests.
- **Consolidated 1.4 Documentation**: Unified 58-page Diátaxis documentation structure, corrected version warning banner semantics, Mermaid architecture diagrams, and static UI mockups.

### Fixed
- Version warning banner redirection to canonical `1.4/` release.
- Clean architecture alignment across coverage and web endpoints.
