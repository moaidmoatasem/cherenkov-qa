...
- [ ] **E0.5f (part 2) — the record step, now the top item.** The battery needs the response the target *actually returned*, not a schema sample. A spec constrains types, not instance values: sampled values make the honest suite fail the conforming control and void every verdict. So the audit is **record-then-perturb** — run the suite green against the live target, capture each response, replay with one axis perturbed. Capture does not exist yet; that is the whole remaining gap between the primitive and a product. Also decide where it runs: too slow for a per-candidate repair loop, affordable for an audit command.
