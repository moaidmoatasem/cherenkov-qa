...
- [x] **E0.5h — productize the audit.** `cherenkov audit` CLI command shipped: CLI surface, `RecordingProxy`-backed identifier extraction, multi-endpoint path correlation via regex matching, broken-impl replay with per-mutant reporting. — `f50a772`
...
- [x] **E0.5i — raise real coverage.** `--identifiers` (JSON file mapping path vars → known IDs) and `--allow-mutations` flags added to `cherenkov verify`. `cherenkov audit` auto-extracts identifiers from `RecordingProxy` traffic via regex. `spec_hypotheses` now generates happy-path probes for templated endpoints when all path vars are supplied. 31 tests pass. — `886648c`
- [x] **E0.5d** — **spec-shape conformance corpus.** Ran `cherenkov verify` against 10 real third-party OpenAPI specs (Stripe, GitHub, Twilio, Kubernetes, OpenAI, Petstore, Slack, Box, SendGrid, DigitalOcean). Results: 3,428 endpoints analyzed, 940 probes planned, 2,488 explicitly dropped, **0 silent drops**, **0 crashes**. Marketing report: `docs/marketing/E0.5d_conformance_corpus.md`. — `bb88ce1`
...
| **T7** | [x] Dual AI routing (`ai/` + `substrate/`) | **Mapped 2026-08-01** (#815, PR #820) — ai/=client layer, substrate/=orchestration, `ai/router.py`=legacy (1 call site). Consolidation plan in `agent_memory/findings_2026-08-01_dual_ai_routing_layers.md`; execution is incremental, not a milestone. |
