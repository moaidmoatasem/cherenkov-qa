...
class RunQwenCodeAgentArgs(BaseModel):
    prompt: str = Field(
        ..., description="The instruction or task to delegate to Qwen Code"
    )
...
def run_qwen_code_agent(args: RunQwenCodeAgentArgs) -> dict:
    ...
    prompt = (
        getattr(args, "prompt", args.get("prompt", ""))
        if isinstance(args, dict)
        else args.prompt
    )
    context = (
        getattr(args, "context", args.get("context", ""))
        if isinstance(args, dict)
        else args.context
    )
    files = (
        getattr(args, "files", args.get("files", []))
        if isinstance(args, dict)
        else args.files
    )
