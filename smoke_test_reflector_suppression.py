...
def _hyp(hid: str, claim_a: str, claim_b: str) -> DivergenceHypothesis:
    return DivergenceHypothesis(
        id=hid, divergence_class=DivergenceClass.D1_SPEC_CODE,
        claim_a=claim_a, claim_b=claim_b,
        predicted_evidence="HTTP 200 where spec promises 404",
        severity=Severity.MEDIUM, endpoint=EP, repro_steps=[],
    )
...
def main() -> int:
    ...
    reflector.ingest_from_reproduction(
        h1, ReproductionResult(hypothesis_id=h1.id, reproduced=False,
    ...
    persisted = "id-RUN2-fresh" not in {
        h.id for h in reflector2.rerank([_hyp("id-RUN3", "spec says 404 for missing pet",
                                              "code returns 200")], endpoint=EP)
    }
