"""FastAPI routes for core Cherenkov QA operations (ingest, run, validate, eject)."""
...
async def ingest_spec_file(file: UploadFile | None = File(None), url: str | None = Form(None), _role=Depends(require_role(Role.reviewer))):
    """Ingest OpenAPI spec file or URL to prepare test generation environment.

    Args:
        file: Optional uploaded OpenAPI spec file.
        url: Optional remote URL to fetch OpenAPI spec from.
        _role: Required authorization role dependency.

    Returns:
        Dictionary containing spec path, list of endpoints, and average richness score.

    Raises:
        HTTPException: 400 Bad Request if neither file nor url provided, 413 if file exceeds size limit, 500 on parse error.
    """
    ...
    spec_path = os.path.join(
        temp_dir,
        f"spec_{run_id}.json" if (file and file.filename and file.filename.endswith(".json")) else f"spec_{run_id}.yaml",
    )
    ...
    try:
        ...
        for ep in ingest_output.endpoints:
            ...
            endpoints.append({"path": ep.path, "method": ep.method, "richness": ep.richness, "missing_elements": missing})
        return {
            "spec_path": spec_path,
            "endpoints": endpoints,
...
async def trigger_pipeline_run(payload: RunPipelinePayload, _background_tasks: BackgroundTasks, _auth=Depends(verify_api_key), _role=Depends(require_role(Role.reviewer))):
    """Trigger background pipeline execution for test generation.

    Args:
        payload: RunPipelinePayload containing spec_path and demo_mode setting.
        _background_tasks: FastAPI BackgroundTasks instance.
        _auth: API key dependency.
        _role: Authorization role dependency.

    Returns:
        Dictionary payload with run_id and launched status string.

    Raises:
        HTTPException: 404 Not Found if spec_path does not exist.
    """
...
async def list_generated_tests():
    """List generated Playwright test files and scenario metadata.

    Returns:
        List of dictionaries containing test file name, scenario ID, endpoint, method, and code.
    """
...
async def validate_test_suite(payload: ValidatePayload, _auth=Depends(verify_api_key), _role=Depends(require_role(Role.reviewer))):
    """Validate generated test suite against live target service URL.

    Args:
        payload: ValidatePayload containing target_url.
        _auth: API key dependency.
        _role: Authorization role dependency.

    Returns:
        Validation results dictionary from validation engine.

    Raises:
        HTTPException: 504 Gateway Timeout on validation timeout, 500 on validation failure.
    """
...
async def eject_test_suite(payload: EjectPayload, _auth=Depends(verify_api_key), _role=Depends(require_role(Role.reviewer))):
    """Eject generated test suite into self-contained standalone directory.

    Args:
        payload: EjectPayload containing output_path.
        _auth: API key dependency.
        _role: Authorization role dependency.

    Returns:
        Dictionary payload containing status, output_path, and list of ejected files.

    Raises:
        HTTPException: 500 Internal Server Error if ejection operation fails.
    """
    try:
