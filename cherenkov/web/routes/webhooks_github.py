...
def verify_signature(payload_body: bytes, secret: str, signature_header: str) -> bool:
    """Verify HMAC SHA256 signature for incoming GitHub webhook payloads.

    Args:
        payload_body: Raw binary request payload body.
        secret: Configured webhook secret string.
        signature_header: Expected X-Hub-Signature-256 header value.

    Returns:
        True if calculated HMAC matches signature_header, False otherwise.
    """
...
async def handle_github_event(
    ...
    """Handle incoming GitHub webhook events and publish coverage comments.

    Args:
        request: FastAPI Request instance.
        x_github_event: Header indicating the GitHub event type.
        x_hub_signature_256: HMAC SHA256 signature header.

    Returns:
        Dictionary status payload indicating event handling outcome.

    Raises:
        HTTPException: 401 Unauthorized if webhook signature is invalid.
    """
