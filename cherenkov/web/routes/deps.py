...
class ConnectionManager:
    ...
    def __init__(self):
        """Initialize empty ConnectionManager instance."""
...
def get_queue() -> HitlQueue:
    """Get the global HitlQueue singleton instance.

    Returns:
        HitlQueue instance for managing human-in-the-loop triage items.
    """
    global _queue
    if _queue is None:
...
def ws_event_callback(type_: str, payload: dict):
    """Callback invoked by pipeline emitters to broadcast events over WebSocket.

    Args:
        type_: Event type string.
