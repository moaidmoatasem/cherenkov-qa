"""FastAPI routes for workspace settings, governance KPIs, and project management."""
...
class NewProjectPayload(BaseModel):
    """Payload model for creating a new project in the workspace."""
    name: str
    target_url: str = ''
    spec_path: str = ''
    repo_type: str = 'new'
...
async def api_get_settings(_auth=Depends(verify_api_key)):
    ...
    Returns:
        Dictionary containing effective settings payload.
    """
...
async def update_settings(body: dict, _auth=Depends(verify_api_key), _role=Depends(require_role(Role.admin))):
    ...
    Args:
        body: Settings updates dictionary.
        _auth: API key dependency.
        _role: Admin role authorization dependency.

    Returns:
        Dictionary containing updated settings payload.
    """
...
async def get_governance():
    """Retrieve governance KPI summary, issues, and model certifications.

    Returns:
        Dictionary payload containing score, issues, defectEscapeRate, falsePositiveRate, and traceability details.
    """
...
async def get_projects():
    ...
    Returns:
        List of project dictionaries containing ID, name, status, pipeline status, and stats.
    """
    def _query():
        conn = _db()
        try:
...
async def get_project(project_id: str):
    ...
    Returns:
        Dictionary containing project information and stats.

    Raises:
        HTTPException: 404 Not Found if project_id does not exist.
    """
    def _query():
        conn = _db()
        try:
...
async def create_project(payload: NewProjectPayload, _role=Depends(require_role(Role.reviewer))):
    """Create a new project entry in the workspace database.

    Args:
        payload: NewProjectPayload object containing name, target_url, spec_path, etc.
        _role: Required reviewer role authorization dependency.

    Returns:
        Dictionary payload containing created project details.
    """
...
async def update_project(project_id: str, payload: dict, _role=Depends(require_role(Role.reviewer))):
    """Update fields of an existing project in the workspace database.
    ...
    Returns:
        Dictionary containing project_id and updated fields.

    Raises:
        HTTPException: 400 Bad Request if no valid fields are provided.
    """
