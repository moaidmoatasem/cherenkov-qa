...
class StartJourneyPayload(BaseModel):
    """Payload model for starting a journey workflow run."""
    spec_path: str
...
async def list_journeys(_: Role = Depends(require_role(Role.viewer))):
    """List registered workflow journeys and identify the default journey.

    Args:
        _: Required viewer role authorization dependency.

    Returns:
        Dictionary containing default journey ID and list of journey definition dictionaries.
    """
...
async def get_journey_run(
    ...
    Args:
        run_id: Journey run identifier string.
        _: Required viewer role authorization dependency.

    Returns:
        Dictionary payload containing run status, verdict, steps, and stage rollups.

    Raises:
        HTTPException: 404 Not Found if run_id is missing.
    """
...
async def get_journey(
    ...
    """Retrieve detailed workflow definition for a specific journey ID.

    Args:
        journey_id: Unique journey identifier string.
        _: Required viewer role authorization dependency.

    Returns:
        Dictionary containing journey steps and stages.

    Raises:
        HTTPException: 404 Not Found if journey_id is invalid.
    """
...
async def get_journey_chains(
    ...
    Returns:
        Dictionary payload containing spec_path and list of detected CRUD chains.

    Raises:
        HTTPException: 400 if spec file cannot be read, 404 if spec file missing.
    """
...
async def start_journey_run(
    ...
    """Launch a new journey run execution.

    Args:
        journey_id: Unique journey identifier string.
        payload: StartJourneyPayload containing spec_path.
        _: Required reviewer role authorization dependency.

    Returns:
        Dictionary containing launched run_id, journey_id, and status.

    Raises:
        HTTPException: 404 Not Found if journey_id or spec_path does not exist.
    """
    _require_journey(journey_id)
