...
class HealingSuggestionPayload(BaseModel):
    """Payload model for requesting a suggest-only healing recommendation."""
    verdict_id: str
...
async def healing_suggestion(
    ...
    """Generate suggest-only repair recommendation for a failed verdict (D7 invariant).

    Args:
        payload: HealingSuggestionPayload object containing verdict_id.
        _auth: API key dependency.

    Returns:
        Dictionary containing suggest-only healing recommendation text.

    Raises:
        HTTPException: 404 Not Found if verdict_id is unknown.
    """
