...
class MaestroRunner(MobileRunnerBase):
    """Executes Maestro flows against a connected device or emulator."""

    def __init__(self, maestro_binary: str = "maestro"):
        self.maestro_binary = maestro_binary

    def run_test(self, yaml_path: str) -> dict:
        """Run one Maestro flow. Returns a verdict dict with ``executed`` set."""
        if self.dry_run:
    ...
    def run_directory(self, directory: str) -> dict:
        """Run every Maestro flow in a directory."""
        if self.dry_run:
    ...
    def health_check(self) -> bool:
        """True when the maestro binary is on PATH and responds to --version."""
        if self.dry_run:
            return True
        try:
            result = subprocess.run(
