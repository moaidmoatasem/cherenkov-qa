...
class TraceReader:
    ...
    def extract_http_response(self, trace_zip_path: str, target_path: str, method: str) -> dict | None:
        ...
        self.log.info("parsing trace.zip file", path=trace_zip_path, target_path=target_path, method=method)
        ...
        try:
            with zipfile.ZipFile(trace_zip_path, "r") as z:
                ...
                network_files = [f for f in z.namelist() if "network" in f or f.endswith(".network")]
                    ...
                    with z.open(net_file) as f:
                        for line in f:
                            try:
                                ...
                                is_net_event = (
                                    event_type == "resource" or 
                                    event_type == "resource-snapshot" or 
                                    "response" in event or 
                                    "snapshot" in event
                                )
                                ...
                                if is_net_event:
                                    ...
                                    req_method = request.get("method", params.get("method", ""))
                                    req_url = request.get("url", params.get("url", ""))
                                    
                                    # Check if this matches our target path and method
                                    if req_method.upper() == method.upper() and target_path in req_url:
                                        status = response.get("status", params.get("status", 0))
                                        ...
                                        if isinstance(post_data, dict):
                                            ...
                                            if sha1:
                                                try:
                                                    with z.open(f"resources/{sha1}") as res_f:
                                                        req_body_raw = res_f.read().decode("utf-8")
                                                except Exception:
                                                    pass
                                            if not req_body_raw:
                                        ...
                                        if not req_body_raw:
                                            req_body_raw = request.get("postDataText", "")
                                        if not req_body_raw:
                                        ...
                                        if isinstance(content, dict):
                                            ...
                                            if sha1:
                                                try:
                                                    with z.open(f"resources/{sha1}") as res_f:
                                                        body_content = res_f.read().decode("utf-8")
                                                except Exception:
                                                    pass
                                                    
                                        if not body_content:
                                            body_data = response.get("body", params.get("responseBody", ""))
                                            if body_data:
                                                if isinstance(body_data, str):
                                                    try:
                                                        body_content = base64.b64decode(body_data).decode("utf-8")
                                                    except Exception:
