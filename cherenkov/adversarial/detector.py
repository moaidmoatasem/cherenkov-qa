...
_TAUTOLOGICAL_PATTERNS = [
    ...
    (
        r"expect\s*\(\s*res\s*\)\.toBeTruthy\s*\(\s*\)",
        "Weak assertion: toTruthy on response object",
    ),
]
...
def scan_test_code(test_code: str) -> list[DetectionResult]:
    ...
    results.extend(
        _scan_patterns(
            test_code,
            _TAUTOLOGICAL_PATTERNS,
            ThreatCategory.TAUTOLOGICAL_TEST,
            Severity.MEDIUM,
        )
    )
    results.extend(
        _scan_patterns(
            test_code,
            _SPEC_MANIPULATION_PATTERNS,
            ThreatCategory.SPEC_MANIPULATION,
            Severity.HIGH,
        )
    )
    results.extend(
