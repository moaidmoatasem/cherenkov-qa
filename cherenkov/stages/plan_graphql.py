...
from cherenkov.sources.graphql.adapter import GraphQLOperation, GraphQLSourceAdapter
...
class GraphQLScenarioPlanner:
    ...
    def _scenarios_for(self, op: GraphQLOperation) -> list[GraphQLScenario]:
        ...
        if (
            "first" in op.variables
            or "after" in op.variables
            or "limit" in op.variables
        ):
            scenarios.append(
                GraphQLScenario(
                    operation_name=op.name,
