...
class DiagnosticsStage:
    ...
    def run(
        ...
        user_prompt = self._build_user_prompt(
            scenario_id, failure_class, error_message, similar_incidents
        )
        ...
        try:
            ...
            hypothesis = parsed.get(
                "hypothesis",
                parsed.get(
                    "suggested_hypothesis", "Could not formulate definitive hypothesis."
                ),
            )
            ...
            self.log.warning(
                "Ollama synthesis failed, using local fallback heuristics", error=str(e)
            )
