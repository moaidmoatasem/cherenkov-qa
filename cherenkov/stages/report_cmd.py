...
def run_report(output: str, diff: str = None) -> int:
    ...
    with open(events_file, "r") as f:
        for line in f:
            ...
            if event.get("stage") == "INGEST" and "skipping low richness endpoint" in event.get("msg", ""):
                ...
                passed = event.get("verdict") in ("AUTO_APPROVE", "HITL") # basic heuristic
    ...
    report_data["passed_scenarios"] = sum(1 for s in report_data["scenarios"] if s["passed"])
    if report_data["total_scenarios"] > 0:
        report_data["success_rate"] = report_data["passed_scenarios"] / report_data["total_scenarios"]
    ...
    print(f"Scenarios: {report_data['passed_scenarios']}/{report_data['total_scenarios']} passed")
