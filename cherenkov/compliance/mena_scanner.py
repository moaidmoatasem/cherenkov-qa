...
class MENAComplianceScanner:
    ...
    def __init__(self, run_id: str | None = None):
        ...
        self.report_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../.cherenkov"))
    ...
    def run_compliance_audit(self, target_url: str, spec_path: str) -> dict:
        ...
        self.log.info("starting MENA cyber security framework compliance audit", target_url=target_url)
        ...
        try:
            ...
            audit_results["tls_enforced"] = target_url.startswith("https") or "127.0.0.1" in target_url or "localhost" in target_url
            
            # Security Header Checks
            audit_results["hsts_present"] = "Strict-Transport-Security" in headers
            audit_results["clickjacking_protection"] = "X-Frame-Options" in headers or "Frame-Options" in headers
            audit_results["mime_sniffing_protection"] = headers.get("X-Content-Type-Options") == "nosniff"

        except Exception as e:
            self.log.warning("failed to actively query target headers, performing static contract-only audit", error=str(e))
        ...
        try:
            ...
            self.log.error("failed to parse OpenAPI spec for compliance check", error=str(e))
            audit_results["errors"].append(f"Spec parsing failed: {e}")

        # 3. Formulate CCSF & FinCSF Compliance Matrices
        score = 0
        if audit_results["tls_enforced"]: score += 20
        if audit_results["hsts_present"]: score += 20
        if audit_results["clickjacking_protection"]: score += 20
        if audit_results["mime_sniffing_protection"]: score += 20
        if audit_results["bearer_auth_defined"]: score += 20

        # Mapping assertions directly to regional monetary clauses
        sama_mapping = {
            "SAMA CCSF Domain 3.1 (Cyber Security Governance)": {
                "status": "COMPLIANT" if audit_results["bearer_auth_defined"] else "NON-COMPLIANT",
            ...
            "SAMA CCSF Domain 3.2 (Cyber Security Operations)": {
                "status": "COMPLIANT" if (audit_results["tls_enforced"] and audit_results["hsts_present"]) else "NON-COMPLIANT",
            ...
            "SAMA CCSF Domain 3.3 (Cyber Security Architecture)": {
                "status": "COMPLIANT" if (audit_results["clickjacking_protection"] and audit_results["mime_sniffing_protection"]) else "NON-COMPLIANT",
        ...
        self.log.info("compliance audit report completed successfully", score=score, path=self.report_path)
        return report