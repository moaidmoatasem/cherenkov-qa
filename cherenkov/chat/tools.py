...
def query_verdicts(
    ...
    try:
        ...
        stats = r.get_stats()
        return {
            "verdicts": stats.get("recent_verdicts", [])[:limit],
            "total": stats.get("total_verdicts", 0),
        }
...
def query_idioms(_pattern: str | None = None, limit: int = 10) -> dict[str, Any]:
    try:
        ...
        stats = r.get_stats()
        return {
            "idioms": stats.get("recent_idioms", [])[:limit],
            "total": stats.get("total_idioms", 0),
        }
    except Exception as e:
...
def explain_divergence(endpoint: str, method: str = "GET") -> dict[str, Any]:
    try:
        ...
        logger.error(
            "explain_divergence failed: endpoint=%s method=%s",
            endpoint,
            method,
            exc_info=e,
        )
...
def run_test(
    ...
    try:
        ...
        if not os.path.exists(effective_spec):
            return {
                "error": f"Spec file not found: {effective_spec}",
                "status": "error",
            }
        ingest = IngestStage("chat-tool").run(effective_spec)
        plan = PlanStage("chat-tool").run(ingest)
        matching = [
            s for s in plan.scenarios if s.endpoint == endpoint and s.method == method
        ]
        return {
            "status": "planned",
            "endpoint": endpoint,
        ...
        logger.error(
            "run_test failed: endpoint=%s method=%s", endpoint, method, exc_info=e
        )
