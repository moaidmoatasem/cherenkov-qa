...
class MetricsCollector:
    ...
    def _init_db(self) -> None:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_timestamp ON stage_metrics(timestamp)"
        )
        conn.commit()
    ...
    def to_prometheus(self) -> str:
        ...
        lines = [
            "# HELP cherenkov_stage_latency_ms Average stage latency in milliseconds",
            "# TYPE cherenkov_stage_latency_ms gauge",
        ]
        for row in summary:
            label = f'stage="{row["stage"]}",run_id="{row["run_id"]}"'
            lines.append(
                f'cherenkov_stage_latency_ms{{{label}}} {row["avg_latency_ms"]:.2f}'
            )
        lines += [
            "# HELP cherenkov_stage_success_rate Stage success rate (0-1)",
            "# TYPE cherenkov_stage_success_rate gauge",
        ]
        for row in summary:
            label = f'stage="{row["stage"]}",run_id="{row["run_id"]}"'
            lines.append(
                f'cherenkov_stage_success_rate{{{label}}} {row["success_rate"]:.4f}'
            )
        return "\n".join(lines) + "\n"
