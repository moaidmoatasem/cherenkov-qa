...
def seed_cmd(spec, suite, profile, ledger_path, as_json):
    ...
    if as_json:
        ...
        click.echo(
            click.style("[SEEDED] ", fg="green", bold=True)
...
@click.option(
    "--output", "-o", default=None, help="Output file path [default: drift-baseline-<id>.json]."
)
@click.option(
    "--ledger",
    "ledger_path",
...
def reconcile_cmd(
    ...
    try:
        ...
        click.echo(
            "Run `cherenkov drift seed --spec ... --suite ...` to create a baseline.", err=True
        )
        sys.exit(1)
...
def _print_report(report) -> None:
    ...
    click.echo(
        "  gate       : " + click.style(report.gate_verdict.value.upper(), fg=gate_color, bold=True)
    )
    ...
    severity_label = {
        ...
        DriftVerdict.PASS: "INFO",
    }
    severity_color = {
        DriftVerdict.FAIL: "red",
        DriftVerdict.WARN: "yellow",
        DriftVerdict.PASS: "white",
    }
...
def _print_loop_result(result) -> None:
    ...
    if result.proposals:
        ...
        for p in result.proposals:
            status = (
                click.style("✔ committed", fg="green")
                if result.committed
