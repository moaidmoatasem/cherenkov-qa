...
class SpecDriftDetector:
    ...
    def _check_schema(
        ...
        if schema_type == "object":
            events.extend(
                self._check_object(endpoint, method, actual, schema, field_path)
            )
        elif schema_type == "array":
            events.extend(
                self._check_array(endpoint, method, actual, schema, field_path)
            )
        elif schema_type in ("string", "integer", "number", "boolean"):
            events.extend(
                self._check_primitive(endpoint, method, actual, schema, field_path)
            )
