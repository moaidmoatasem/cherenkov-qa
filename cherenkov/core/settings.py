"""
cherenkov/core/settings.py — Central system configuration settings and loader.
"""
...
class CherenkovSettings(BaseSettings):
    """Global system configuration model loaded from environment variables and cherenkov.toml."""
    ...
    def settings_customise_sources(
        ...
        """Customise settings sources to include cherenkov.toml loading.

        Args:
            settings_cls (type[BaseSettings]): BaseSettings subclass.
            init_settings (PydanticBaseSettingsSource): Init kwargs settings source.
        ...
        Returns:
            tuple[PydanticBaseSettingsSource, ...]: Custom source sequence.
        """
        class TomlSource(PydanticBaseSettingsSource):
            """Custom settings source for loading configuration from cherenkov.toml."""

            def __init__(self, settings_cls):
                """Initialize TomlSource.
            ...
            def get_field_value(self, field, field_name: str) -> tuple[Any, str, bool]:
                """Retrieve field value from TOML mapped dictionary.

                Args:
                    field: Field definition.
                    field_name (str): Setting field name.

                Returns:
                    tuple[Any, str, bool]: Value, field_name, and is_complex tuple.
                """
            ...
            def prepare_field_value(self, field_name: str, field, value: Any, value_is_complex: bool) -> Any:
                """Prepare field value before setting on model.

                Args:
                    field_name (str): Setting field name.
                    field: Field object.
                    value (Any): Raw value.
                    value_is_complex (bool): True if complex type.

                Returns:
                    Any: Processed field value.
                """
                return value

            def __call__(self) -> dict[str, Any]:
                """Return mapped TOML settings dictionary.

                Returns:
                    dict[str, Any]: Mapped settings key-values.
                """
                return self.mapped
    ...
    def TIERS(self) -> dict[str, dict[str, str]]:  # noqa: N802
        """Return dict of configured AI model tiers.

        Returns:
            dict[str, dict[str, str]]: Mapping of tier names to provider and model settings.
        """
        return {
            "small": {
    ...
    def __init__(self, **data):
        """Initialize CherenkovSettings model instance.

        Args:
            **data: Field values passed directly to init.
        """
        super().__init__(**data)
    ...
    def to_dict(self):
        """Dump settings to dictionary format.
    ...
    def detect_ollama_device(self, run_id: str | None = None) -> str:
        ...
        Args:
            run_id (str | None, optional): Run correlation ID for logging. Defaults to None.

        Returns:
            str: "GPU", "CPU", or "UNKNOWN" processor type.
        """
        now = time.monotonic()
        if (
...
def get_settings() -> CherenkovSettings:
    """Return the global CherenkovSettings singleton instance.

    Returns:
        CherenkovSettings: Global settings singleton instance.
    """
...
def reset_settings() -> None:
    """Reset the global CherenkovSettings singleton instance.
