"""
cherenkov/core/stage_executor.py — Stage execution engine, retry ladder, and circuit breaker.
"""
...
class CircuitBreaker:
    ...
    def __init__(self, threshold: int = 2):
        ...
        Args:
            threshold (int, optional): Failure threshold before breaker trips. Defaults to 2.
        """
    ...
    def record_failure(self):
        """Increment failure count and trip breaker if threshold reached.
    ...
    def reset(self):
        """Reset error count and clear tripped state.
...
class StageExecutor:
    ...
    def __init__(self, breaker: CircuitBreaker, logger: Any):
        """Initialize StageExecutor instance.

        Args:
            breaker (CircuitBreaker): Circuit breaker instance.
            logger (Any): Logger object for emitting stage execution events.
        """
    ...
    def execute(
        ...
        """Execute a pipeline stage function with exponential backoff retries.

        Args:
            stage_name (str): Stage identifier name string.
            stage_func (Callable[[], Any]): Stage execution function.
            fallback_factory (Callable[[], Any]): Factory generating fallback result on failure.

        Returns:
            Any: Validated stage output object or fallback.
        """
        attempts = 0
        max_attempts = 3
    ...
    def execute_with_vlm_retry(
        ...
        """Shared retry ladder for visual/perf stages.

        Args:
            stage_name (str): Stage identifier.
            slices (list[Any]): List of slice target items.
            stage_factory (Callable[[], Any]): Factory producing stage instances.
            run_slice (Callable): Function to execute single slice.
            fallback_report_factory (Callable): Factory for fallback reports on failure.
            report_type (type[Any]): Expected report contract type class.
            contract_error_msg (str): Error message template for validation failures.

        Returns:
            list[Any]: List of validated report objects.
        """
        stage = stage_factory()
        results: list[Any] = []
