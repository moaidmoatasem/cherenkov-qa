...
class StalenessReport:
    """Report detailing test file staleness status relative to spec hash."""
...
class StalenessManifest:
    ...
    def __init__(self, manifest_path: Path = _MANIFEST_PATH):
        """Initialize StalenessManifest.

        Args:
            manifest_path (Path, optional): Path to test manifest JSON file. Defaults to _MANIFEST_PATH.
        """
    ...
    def record(
        ...
        Args:
            ...
            test_files (Sequence[str]): List of generated test file paths.
    ...
    def check(self) -> StalenessReport:
        ...
        Returns:
            StalenessReport: Report indicating whether test artifacts are stale or up-to-date.
        """
        data = self._load()
        if not data:
    ...
    def info(self) -> dict:
        ...
        Returns:
            dict: Raw dictionary content of staleness manifest.
        """
        return self._load()
