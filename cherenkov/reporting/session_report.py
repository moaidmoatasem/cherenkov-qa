...
class SessionReportBuilder:
    ...
    def add_http_verdict(
        ...
        self._findings.append(SessionFinding(
            kind="http_verdict",
            severity=severity,
            ...
            suggestions=[
                ...
                f"Check application logs for errors triggered by this request",
            ],
        ))
        return self
    ...
    def add_explorer_finding(self, finding: Any) -> "SessionReportBuilder":
        ...
        kind_val = finding.kind.value if hasattr(finding.kind, "value") else str(finding.kind)
        self._findings.append(SessionFinding(
            kind="explorer",
            severity=severity,
    ...
    def add_visual_report(self, report: Any) -> "SessionReportBuilder":
        ...
        for gate in getattr(report, "gates", []):
            ...
            if not gate.passed:
                vlm_gate = next(
                    (g for g in getattr(report, "gates", []) if g.gate == "vlm_semantic"),
                    None,
                )
                kind = "unknown"
