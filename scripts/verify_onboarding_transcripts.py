...
def _real_flags_by_command(cli) -> dict[str, set[str]]:
    ...
    def visit(cmd, prefix: str) -> None:
        """Traverse Click command hierarchy to collect valid flag names.

        Args:
            cmd: Click Command or Group instance.
...
def main() -> int:
    """Verify onboarding transcripts against the live CHERENKOV Click CLI commands and flags.

    Parses CLI invocations in session markdown documents and confirms that all command paths
    and options exist and match the live CLI registration without drift.

    Returns:
        int: 0 if all documented commands and flags are valid, 1 if drift errors are detected.
    """
