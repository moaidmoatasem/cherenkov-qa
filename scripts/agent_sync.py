...
def cmd_before(task_type: str, budget: int | None = None, source: str = "cherenkov"):
    ...
    Args:
        task_type: Type identifier of task being initiated.
        budget: Optional per-session token budget.
        source: System or agent source identifier string.
...
def cmd_after(summary: str):
    ...
    Args:
        summary: Summary string of work completed during session.
...
def cmd_memory(action: str, args: list) -> None:
    ...
    Args:
        action: Memory action string (list, promote, search, or status).
        args: Command-line argument strings list.
...
def cmd_log(log_type: str, message: str):
    ...
    Args:
        log_type: Classification string of finding (e.g., decision, finding, pitfall).
...
def cmd_token(action_type: str | None, count: int | None, item: str | None):
    ...
    Args:
        action_type: Category of action (prompt, generate, read, or search).
        count: Number of tokens consumed.
        item: Optional item or file name description.

    Returns:
        None or calls cmd_status.
    """
...
def cmd_compact(force: bool = False):
    ...
    Args:
        force: Whether to force context compaction regardless of session count.
...
def cmd_experience_query(
    ...
    Args:
        ...
        sort: Optional sort ordering string.
...
def main():
    """Main CLI entry point for agent_sync SDD session tracking.
