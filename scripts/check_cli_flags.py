...
# Files that invoke the CLI on a user's behalf. A wrong flag here is worse than
# a wrong flag in prose: it fails someone's pipeline, and (in the Jenkins case)
# the failure is caught and reported as a conformance finding rather than a
# usage error, so it looks like the API under test is broken.
_CI_SUFFIXES = (".yml", ".yaml", ".groovy", ".sh")

_INVOKE_RE = re.compile(
    r"(?:python3?\s+-m\s+cherenkov|\./bin/cherenkov|(?<![\w./-])cherenkov)"
    r"\s+([a-z][\w-]*)"
)
_FLAG_RE = re.compile(r"(?<![\w-])(--[a-z][\w-]*)")
# Groovy/bash incremental command building: `cmd += " --export-jira"`.
_APPEND_RE = re.compile(r"""^\s*\w+\s*\+=\s*["'][^"']*--[a-z]""")


def _ci_files(repo_root: str) -> list[str]:
    """CI templates and the composite action that invoke the CLI."""
    import glob

    files = [
        f
        for f in glob.glob(os.path.join(repo_root, "ci", "**", "*"), recursive=True)
        if os.path.isfile(f) and f.endswith(_CI_SUFFIXES)
    ]
...
def _iter_ci_flag_uses(text: str):
    """Yield (line_no, command, flag) for every CLI flag used in a CI file.

    Handles the three shapes these files actually use, not just the one-liner:
      * a single-line invocation (`cherenkov validate --target ... --quiet`)
      * a shell continuation, where flags trail on later lines after a `\\`
      * incremental building, where a variable holding the command is appended
        to (`cmd += " --export-jira"`)
    """
    ...
    for line_no, raw in enumerate(text.splitlines(), 1):
        line = raw.rstrip()
        invoke = _INVOKE_RE.search(line)

        if invoke:
            current = invoke.group(1)
            scan_from = invoke.end()
            for flag in _FLAG_RE.findall(line[scan_from:]):
                yield line_no, current, flag
        elif current and (continuing or _APPEND_RE.match(line)):
            for flag in _FLAG_RE.findall(line):
                yield line_no, current, flag

        # A trailing backslash continues the current invocation onto the next
        # line. Anything else ends it, so unrelated later commands in the same
        # file are not attributed to it.
        continuing = bool(current) and line.endswith("\\")
            ...
            for line_no, line in enumerate(f, 1):
                for match in inline_re.finditer(line):
                    ...
                    if command not in real:
                        ...
                        checked += 1
    # 3. Check CI templates and action.yml, which invoke the CLI for real users
    for ci_file in _ci_files(_REPO_ROOT):
        with open(ci_file, encoding="utf-8", errors="ignore") as f:
            ci_text = f.read()
        rel = os.path.relpath(ci_file, _REPO_ROOT)
        for line_no, command, flag in _iter_ci_flag_uses(ci_text):
            if flag in _HELP_FLAGS:
                continue
            real_opts = real.get(command, set())
            if command not in real:
                errors.append(
                    f"{rel}:{line_no} invokes command '{command}' which doesn't exist."
                )
                continue
            checked += 1
            if flag not in real_opts:
                errors.append(
                    f"{rel}:{line_no} passes '{flag}' to '{command}', which has no "
                    f"such option (has: {sorted(real_opts) if real_opts else 'none'})"
                )

    print(f"Checked {checked} documented flags across the CLI reference and CI templates.")
    if errors:
        print(f"\n[FAIL] {len(errors)} flag drift issue(s):")
        for e in errors:
            print(f"  * {e}")
        print(
            "\nFix the docs and CI templates so every flag they pass is a real "
            "Click option."
        )
        return 1
