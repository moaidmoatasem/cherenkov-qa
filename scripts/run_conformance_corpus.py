...
def load_spec(path: Path) -> dict[str, Any]:
    """Load and parse OpenAPI specification file (YAML or JSON).

    Args:
        path: Path instance pointing to spec file.
...
def analyze_spec(spec_path: Path, max_probes: int = 100000) -> dict[str, Any]:
    """Analyze OpenAPI spec and compute coverage metrics for target probes.

    Args:
        spec_path: Path instance pointing to spec file.
        max_probes: Maximum number of probes to analyze.

    Returns:
        Dictionary containing spec name, total endpoints, probes, and coverage statistics.
    """
...
def main() -> None:
    """Execute conformance corpus benchmark across OpenAPI/Swagger specifications.

    Iterates through all specification files in the corpus directory, analyzes
    endpoint extraction, planned probes, and drop reasons, verifies the zero-silent-drop
    invariant, and writes summary benchmark statistics to JSON.

    Raises:
        AssertionError: If any silent drops are detected during spec analysis.
        SystemExit: If the corpus directory is missing or contains no spec files.
    """
