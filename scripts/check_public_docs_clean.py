...
def check_file(path: Path, strict: bool) -> list[tuple[str, int, str, str]]:  # noqa: ARG001
    """Check a documentation file for forbidden internal tokens and markers.

    Args:
        path: Path to the Markdown file being validated.
        strict: Flag indicating whether strict validation mode is enabled.

    Returns:
        list[tuple[str, int, str, str]]: List of (file, line_number, severity, message) tuples.
    """
    violations = []
    try:
...
def main() -> int:
    """CLI entry point to validate public documentation files against forbidden internal tokens.

    Returns:
        int: 0 if all docs are clean or only warnings in non-strict mode, 1 if violations found.
    """
