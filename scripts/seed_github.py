...
def get_token():
    """Retrieve GitHub token using git credential fill.

    Returns:
        GitHub token password string.
    """
    p = subprocess.run(
        ["git", "credential", "fill"],
...
def call(method, path, body=None):
    """Execute REST call to GitHub API endpoint.

    Args:
        method: HTTP method string (e.g. GET, POST).
        path: API path string.
        body: Optional payload body dictionary.

    Returns:
        Tuple containing status code integer and parsed response data.
    """
...
def get_all(path):
    """Retrieve all paginated items for a GitHub API path.

    Args:
        path: API endpoint path string.

    Returns:
        List of all retrieved resource dictionaries.
    """
    out, page = [], 1
    while True:
...
def ensure_labels() -> None:
    """Ensure all required repository issue labels and epoch tags exist on GitHub.

    Queries the GitHub repository labels API and creates any missing labels defined
    in the LABELS configuration table with their designated colors and descriptions.
    """
...
def ensure_milestones():
    """Ensure all required epoch milestones exist on GitHub repository.

    Returns:
        Dictionary mapping epoch indices to GitHub milestone numbers.
    """
...
def task(code, title, body, area, typ, accept):
    """Format task specification into structured issue dictionary.

    Args:
        code: Task code identifier string.
        title: Task title string.
        body: Task description body string.
        area: Optional area label string.
        typ: Issue type label string.
        accept: List of acceptance criteria strings.

    Returns:
        Structured task dictionary containing code, title, body, and labels.
    """
...
def main():
    """Seed GitHub repository with labels, milestones, and phase issues.
    ...
    def make_issue(title, body, labels, milestone):
        """Create a new GitHub issue if it does not already exist.
        ...
        Returns:
            Created or existing issue number integer.
        """
