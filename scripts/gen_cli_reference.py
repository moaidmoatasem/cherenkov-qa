...
def cmd_to_heading(cmd: list[str]) -> str:
    """Format command list into Markdown heading string.

    Args:
        cmd: List of command words (e.g. ['cherenkov', 'validate']).
...
def generate() -> str:
    """Generate Markdown text for CLI reference section.

    Returns:
        Generated Markdown reference document string.
    """
    lines: list[str] = [
...
def main() -> int:
    """Main CLI entry point for generating and writing CLI reference Markdown file.
