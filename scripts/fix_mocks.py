...
content = content.replace("PIPELINE_STREAMING_TESTS[currentTestIndex] || PIPELINE_STREAMING_TESTS[0]", "realTestQueue[currentTestIndex] || { endpoint: 'Waiting for stream...', code: '', agent: 'System' }")
...
with open(overview_path, 'r') as f:
    content = f.read()
content = content.replace("import { MOCK_OVERVIEW } from '../mockData';", "import { fetchOverview } from '../lib/api';")
...
with open(healing_path, 'r') as f:
    content = f.read()
content = content.replace("import { INITIAL_FAILURES } from '../mockData';", "import { fetchFailures } from '../lib/api';")
content = re.sub(
...
with open(truth_path, 'r') as f:
    content = f.read()
content = content.replace("import { MOCK_TRUTH_MAP } from '../mockData';", "import { fetchTruthMap } from '../lib/api';")
content = re.sub(
...
with open(signals_path, 'r') as f:
    content = f.read()
content = content.replace("import { MOCK_SIGNALS } from '../mockData';", "import { fetchSignals } from '../lib/api';")
content = re.sub(
...
with open(gov_path, 'r') as f:
    content = f.read()
content = content.replace("import { MOCK_GOVERNANCE } from '../mockData';", "import { fetchGovernance } from '../lib/api';")
content = re.sub(
...
with open(memory_path, 'r') as f:
    content = f.read()
content = content.replace("import { MOCK_IDIOMS, MOCK_PAIRING } from '../mockData';", "import { fetchMemory } from '../lib/api';")
content = re.sub(
...
with open(author_path, 'r') as f:
    content = f.read()
content = content.replace("import { MOCK_MENTOR_IDIOMS } from '../mockData';", "import { fetchMemory } from '../lib/api';")
content = re.sub(
