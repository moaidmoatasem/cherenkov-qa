...
def run_validation(
    ...
    """Probe every route in a spec against a live base URL.

    Args:
        spec_path (str): Path to the OpenAPI spec (YAML or JSON).
        target_base (str): Base URL of the API to probe.
        strict (bool): Reserved; currently unused by the comparison.

    Returns:
        dict[str, Any]: ``passed``, the list of ``divergences``, a human
        ``summary`` and the per-route ``checks``.
    """
