...
def run_rag_smoke_tests():
    ...
    results = rag.query_similar_incidents(
        error_message="Expected 'id' key was missing",
        limit=2
    )
    ...
    for res in results:
        print(f"  - Incident: {res['id']} | Similarity: {res['similarity']} | Msg: {res['error_message']}")

    # Assert that the list is sorted by similarity descending
    if len(results) > 1:
        assert results[0]["similarity"] >= results[1]["similarity"], "Incidents are not sorted by similarity descending."
