...
def test_c8_explorer():
    ...
    def mock_ui_probe(url):
        if "js_err" in url:
            return [(ExplorerFindingKind.JS_ERROR, "Uncaught ReferenceError: x is not defined", "at index.js:10")]
        return []
    ...
    check("DivergenceClass D2_CODE_PROD assigned to 5xx/slow/unreachable", DivergenceClass.D2_CODE_PROD in classes)
    check("DivergenceClass D5_SPEC_PROD assigned to 4xx client errors", DivergenceClass.D5_SPEC_PROD in classes)
    check("DivergenceClass D3_UI_SPEC assigned to JS errors", DivergenceClass.D3_UI_SPEC in classes)
...
def test_c9_intent_author():
    ...
    spec = author.parse("verify cart discount apply with SAVE20", target_url="http://localhost:8000")
    check("Intent Spec parsed correctly from router", spec.title == "Add to Cart and Check Discount")
    ...
    fallback_spec = broken_author.parse("smoke test", target_url="http://localhost:8000")
    ...
    check("Emitted code contains playwright imports", 'import { test, expect } from "@playwright/test"' in pw_code)
    check("Emitted code uses selector-free getByLabel role locator", 'page.getByLabel("Promo Code")' in pw_code)
    check("Emitted code uses selector-free getByRole button locator", 'page.getByRole("button", { name: "Apply Discount" })' in pw_code)
    check("Emitted code contains expected assertions", 'expect(page.getByText("Discount applied successfully")).toBeVisible()' in pw_code)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        check("Test file has correct filename extension", file_path.name.endswith(".spec.ts"))
...
def test_c10_digest_and_triage():
    ...
    check("Digest ranked critical item first", digest.items[0].severity == Severity.CRITICAL)
    check("Digest items contain Reflector idioms source", any(it.source == "idiom" for it in digest.items))

    rendered_digest = digest.render()
    check("Rendered digest lists target URL", "http://localhost:8000" in rendered_digest)
    ...
    check("FailureClass.AUTH_EXPIRY triaged as ENV", env_triage.category == TriageCategory.ENV)

    flaky_triage = triage_engine.triage("scen-2", FailureClass.DETERMINISTIC_FAILURE, retried_pass=True)
    check("Passed-on-retry forces category to FLAKY", flaky_triage.category == TriageCategory.FLAKY)
    ...
    check("BUG category nudged to INTENDED via Reflector memory", nudged_triage.category == TriageCategory.INTENDED)
